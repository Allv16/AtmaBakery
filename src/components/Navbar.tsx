...
export function Navbar() {
  ...
  return (
    <div className="navbar bg-white  px-24 font-serif w-full mx-auto fixed top-0 left-0 right-0 z-50 shadow-md">
      ...
      <div className="navbar-center gap-5 xl:gap-10 xl:text-lg cursor-pointer">
        <a
          className={`font-bold text-black hidden lg:block ${!pathname.includes('about') && !pathname.includes('invoice') && !pathname.includes('how-to-order') && !pathname.includes('login') && !pathname.includes('register') && !pathname.includes('profile') && !pathname.includes('history') ? 'border-b-2 border-primary' : ''}`}
          onClick={() => navigate('/')}
        >
          HOME
        </a>
      ...
      <div className="navbar-end">
        {
          profileUrl != null ? (
            <div className="dropdown dropdown-bottom dropdown-end">
              <div tabIndex={0}>
                <img src={profileUrl} className="w-14 h-14 object-cover rounded-full cursor-pointer" />
              </div>
              <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
                ...
                <li>
                  <a className="text-lg text-red-600" onClick={handleLogout}>Logout</a>
                </li>
              </ul>
            </div>
          ) : (
            <>
              ...
              <button className="btn btn-sm btn-primary font-sans lg:hidden text-xs" onClick={() => navigate("/login")}>
                Login/SignUp
              </button>
            </>
          )
