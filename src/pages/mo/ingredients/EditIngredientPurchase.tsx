...
import { editIngredientPurchase, getIngredientPurchaseById } from '../../../lib/repository/IngredientPurchaseRepository';
...
const EditIngredientPurchase: React.FC = () => {
    ...
    const { data: ingredientsData, error: ingredientsError, isLoading: ingredientsIsLoading } = getAllIngredients();
            ...
            <div className="bg-white shadow-default p-6">
                ...
                {data && (
                    <form className="flex flex-col gap-6" onSubmit={(e) => {
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-6">
                            <label className="font-medium text-gray-800 w-full max-w-xs">Ingredient</label>
                            <select
                            ...
                            >
                                ...
                                {ingredientsData && ingredientsData.length > 0 ? (
                                    ingredientsData.map((ingredients: IIngredients) => (
                                        <option key={ingredients.id_bahan_baku} value={ingredients.id_bahan_baku}>
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-6">
                            <label className="font-medium text-gray-800 w-full max-w-xs">Date Of Purchase</label>
                            <input type="date" className="input w-full max-w-md" name="tanggal_pembelian"
                                onChange={handleChange} value={input.tanggal_pembelian} required />
                        </div>

                        <div className="flex flex-wrap items-center justify-center gap-6">
                            <label className="font-medium text-gray-800 w-full max-w-xs">Purchase Quantity</label>
                            <input type="number" placeholder="Enter Purchase Quantity" className="input w-full max-w-md" name="jumlah_pembelian"
                                min="0" onChange={handleChange} value={input.jumlah_pembelian} required />
                        </div>

                        <div className="flex flex-wrap items-center justify-center gap-6">
                            <label className="font-medium text-gray-800 w-full max-w-xs">Purchase Price</label>
                            <input type="number" placeholder="Enter Purchase Price" className="input w-full max-w-md" name="harga_beli"
                                min="0" onChange={handleChange} value={input.harga_beli} required />
                        </div>
                ...
                <dialog id="edit_modal" className="modal" hidden>
                    <div className="modal-box">
                        ...
                        <p className="py-4">Are you sure you want to edit this ingredient purchase?</p>
                        <div className="flex justify-end">
                            <form method="dialog" className="flex space-between gap-3">
                                <button id="cancel_edit">Cancel</button>
                                <button id="confirm_edit" className="btn btn-primary" onClick={() => handleSubmit()}>Save</button>
                            </form>
                        </div>
                    </div>
                </dialog>
