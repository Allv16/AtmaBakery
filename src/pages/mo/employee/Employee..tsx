...
const Employee: React.FC = () => {
    //API CALL
    const { data, error, isLoading } = getAllEmployee();
    const [search, setSearch] = React.useState("");

    const dataFiltered = data?.filter((item: IEmployee) => {
        return item.nama_karyawan.toLowerCase().includes(search.toLowerCase());
    });
    console.log(search);
            ...
            <MOWrapper>
                ...
                {data && (
                    <div className="rounded-sm border border-stroke bg-white shadow-default p-4 overflow-x-auto">
                        <EmployeeTable employeeData={dataFiltered} />
                    </div>
                )}
            </MOWrapper>
        </>
